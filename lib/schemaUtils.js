...
const async = require('async'),
  ...
  testSuite = require('./pmTestSuite.js'),
  ...
  convertRequestToItem: function(openapi, operationItem, components, options, schemaCache, variableStore) {
    ...
    _.forEach(reqParams.query, (queryParam) => {
      this.convertToPmQueryParameters(queryParam, REQUEST_TYPE.ROOT, components, options, schemaCache)
        .forEach((pmParam) => {
          // pmTestSuite - Modify the request query params for the test suites
          pmParam = testSuite.overwriteRequestQueryParamByTests(pmParam, operation, operationItem, options);
    ...
    // pmTestSuite - Modify the request paths variables for the test suites
    reqParams.path = testSuite.overwriteRequestPathByTests(reqParams.path, operation, operationItem, options);
    ...
    _.forEach(reqParams.header, (header) => {
      // pmTestSuite - Modify the request header for the test suite
      header = testSuite.overwriteRequestHeaderByTests(header, operation, operationItem, options);
    ...
    if (reqBody) {
      ...
      // pmTestSuite - Modify the request body for the test suite
      pmBody.body = testSuite.overwriteRequestBodyByTests(pmBody.body, operation, operationItem, options);
      // pmTestSuite - Assign PM vars based on the request body for the test suite
      pmTestVars = testSuite.assignVariablesByRequestBody(pmBody.body, operation, operationItem, options);
    ...
    // pmTestSuite - Generate tests for the response
    pmTests = testSuite.convertResponsesToPmTest(operation, operationItem, components, options, schemaCache);

    // Merge pmTests array
    const pmCombinedTests = [...pmTests, ...pmTestVars];
