  ...
  */
  convertRequestToItem: function(openapi, operationItem, components, options, schemaCache, variableStore) {
    ...
    if (reqBody) {
      ...
      // Modify the request body for the test suites
      pmBody.body = this.overwriteRequestBodyByTests(pmBody.body, operation, operationItem, options);
          ...
          if (contentType === APP_JSON) {
            ...
            if (_.get(testSuiteSettings, 'responseChecks.schemaValidation.enabled') && content.schema) {
              ...
              try {
                ...
                let resolvedSchema = deref.resolveRefs(schemaContent, 'response', components,
   ...
   * @param {*} postman requestBody object
   * @param {*} operation openapi operation object
   * @param {*} operationItem - The operation item to get relevant information for the test generation
   * @param {object} options - a standard list of options that's globally passed around. Check options.js for more.
  ...
  overwriteRequestBodyByTests: function (requestBody, operation, operationItem, options) {
    options = _.merge({}, defaultOptions, options);
    let testRequestBody = {...requestBody}, // clone requestBody before manipulation
      testSuite = {},
      testSuiteSettings = {},
      overwriteValues = [],
    ...
    if (testSuite.overwriteRequests) {
      ...
      testSuiteOverwriteRequest = testSuiteOverwriteRequests.find(or => or.openApiOperationId === operation.operationId);

      if (testSuiteOverwriteRequest && testSuiteOverwriteRequest.overwriteKeyValues) {
        overwriteValues = testSuiteOverwriteRequest.overwriteKeyValues;
        ...
        testRequestBody.raw = JSON.stringify(bodyData, null, 4)
      }
    }
