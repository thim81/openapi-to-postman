          ...
          if (contentType === APP_JSON) {
            ...
            if (testSuite.contentChecks) {
              let testSuiteContentChecks = testSuite.contentChecks,
                // Get the checkRequestBody settings for the operationId or openApiOperation
                setContentChecksForOperation = testSuiteContentChecks.find((or) => {
                  return this.isMatchOperationItem(operationItem, or);
                });

              if (setContentChecksForOperation && setContentChecksForOperation.checkRequestBody) {
                let requestBodyChecks = setContentChecksForOperation.checkRequestBody;
                requestBodyChecks.forEach((check) => {
                  ...
                  if (check.value) {
                    ...
                    if (check.value.includes('{{') && check.value.includes('}}')) {
                      checkValue = 'pm.environment.get("' + check.value.replace('{{').replace('}}') + '"))';
                    }
